import { Flex, Box, Group, Avatar, Text, rem } from '@mantine/core';
  ...
  return (
    <Flex mt={mt} gap={6} align="center">
      ...
      <Flex direction="column">
        <Link to={{ pathname: `/${username}` }} className='textLink displayBlockLink'>
          <Group gap={0}>
            <Text size="13.5px" fw="500">{name} {lastname}</Text>
            {verified && 
        ...
        <Box w={170}>
          <Link to={{ pathname: `/${username}` }} className='textLink displayBlockLink'>
            <Text size="11px" mt={3} mb={4} truncate="end">{username}</Text>
          </Link>
